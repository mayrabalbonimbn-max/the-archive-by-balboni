...
const desktopNavItems = [
  ...
  { to: '/today', label: 'Hoje', Icon: HomeIcon },
  { to: '/memories', label: 'Memórias', Icon: BookIcon },
  { to: '/calendar', label: 'Calendário', Icon: BookIcon },
  { to: '/profile', label: 'Perfil', Icon: UserIcon },
  { to: '/friends', label: 'Conexões', Icon: FriendsIcon },
  { to: '/notifications', label: 'Notificações', Icon: BellIcon },
  { to: '/diary', label: 'Diário', Icon: BookIcon },
  { to: '/collections', label: 'Coleções', Icon: FolderIcon },
  { to: '/library', label: 'Biblioteca', Icon: LibraryIcon },
  { to: '/photos', label: 'Fotografias', Icon: LibraryIcon },
  { to: '/stats', label: 'Estatísticas', Icon: FolderIcon },
  { to: '/saved', label: 'Salvos', Icon: BookmarkIcon },
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex flex-col items-start h-screen sticky top-0 py-6 px-4 w-[220px] xl:w-[256px] shrink-0 border-r border-dark-border/50 overflow-y-auto">
        ...
        >
          <img src="/icons/icon-192.png" alt="The Archive by Balboni" className="w-10 h-10 rounded-full object-cover shrink-0 group-hover:opacity-85 transition-opacity" />
        ...
        <div className="mt-auto w-full space-y-1 shrink-0">
          <button
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-dark-hover cursor-pointer transition-all duration-200 w-full group"
          >
          ...
          <button
            onClick={onLogout}
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-dark-hover transition-all duration-200 w-full text-dark-label hover:text-red-400 group"
            title="Sair"
          >
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-40">
          ...
          <div className="absolute left-0 top-0 h-full w-64 bg-dark-card border-r border-dark-border flex flex-col pt-16 overflow-hidden">
            {/* Profile header */}
            ...
            <nav className="flex flex-col gap-0.5 px-3 py-3">
              <NavLink to="/" end onClick={close} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                {({ isActive }) => (<><span className={isActive ? 'text-brand-rose' : 'text-dark-muted'}><HomeIcon filled={isActive} /></span><span className="text-[15px]">Início</span>{isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-brand-rose" />}</>)}
              </NavLink>
              ...
              <button
                ...
                className="nav-item"
              >
                <span className="text-dark-muted"><ExploreIcon /></span>
                <span className="text-[15px]">Explorar</span>
              </button>

              <button
                onClick={() => { close(); onCompose?.() }}
                className="nav-item"
              >
                <span className="text-dark-muted"><PencilIcon /></span>
                <span className="text-[15px]">Criar</span>
              </button>

              <NavLink to="/archive" onClick={close} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                {({ isActive }) => (<><span className={isActive ? 'text-brand-rose' : 'text-dark-muted'}><ArchiveIcon filled={isActive} /></span><span className="text-[15px]">Arquivo</span>{isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-brand-rose" />}</>)}
              </NavLink>

              <NavLink to="/profile" onClick={close} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                {({ isActive }) => (<><span className={isActive ? 'text-brand-rose' : 'text-dark-muted'}><UserIcon filled={isActive} /></span><span className="text-[15px]">Perfil</span>{isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-brand-rose" />}</>)}
              </NavLink>

              <NavLink to="/settings" onClick={close} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`}>
                {({ isActive }) => (<><span className={isActive ? 'text-brand-rose' : 'text-dark-muted'}><SettingsIcon filled={isActive} /></span><span className="text-[15px]">Ajustes</span>{isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-brand-rose" />}</>)}
              </NavLink>
            </nav>

            <div className="mt-auto px-3 pb-8 pt-3 border-t border-dark-border/50">
              <button
